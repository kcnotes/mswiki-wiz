...
pub fn to_simple_json(node: &WzNode) -> Result<serde_json::Value> {
  if node.children.is_empty() {
      match &node.object_type {
          WzObjectType::Value(value_type) => return Ok(value_type.clone().into()),
          WzObjectType::Property(WzSubProperty::PNG(inner)) => return Ok(to_value(inner).unwrap()),
          WzObjectType::Property(WzSubProperty::Sound(inner)) => return Ok(to_value(inner).unwrap()),
